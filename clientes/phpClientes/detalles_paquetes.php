...
if($id == '' || $token == '') {
    ...
    if($token == $token_tmp) {
        ...
        $sql = $con->prepare("SELECT count(id) FROM paquetes WHERE id=? AND activo=1");
        $sql->execute([$id]);
        if($sql->fetchColumn() > 0) {

            $sql = $con->prepare("SELECT nombre, descripcion, precio, descuento FROM paquetes WHERE id=? AND activo=1
        LIMIT 1");
        $sql->execute([$id]);
        ...
        $dir_images = '../images/paquetes/' . $id . '/';

        $rutaImg = $dir_images . "combos_$id.png";
        ...
        while(($archivo = $dir->read()) != false) {
            if($archivo != "combos_$id.png" && (strpos($archivo, 'png') || strpos($archivo, 'jpeg'))) {
    ...
    <header class="d-flex justify-content-between align-items-flex-start">
     <a class="navbar-brand" href="promociones.php" class="return"><svg  xmlns="http://www.w3.org/2000/svg"  width="60"  height="50"  viewBox="0 0 24 24"  fill="none"  stroke="currentColor"  stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-arrow-left"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M5 12l14 0" /><path d="M5 12l6 6" /><path d="M5 12l6 -6" /></svg></a>
        <ul class="nav nav-underline align-items-flex-start">
                <li class="nav-item">
                    <a class="nav-link" href="checkout_paquetes.php">| Combo Pedidos<span id="num_card" class="badge bg-secondary"><?php echo $total_items;?></span> |</a>
  ...
  <button class="carousel-control-next" type="button" data-bs-target="#carouselImages" data-bs-slide="next">
    ...
    <span class="visually-hidden">Next</span>
        ...
        <div class="col-md-6 order-md-2">
            ...
            <?php if($descuento > 0) { ?>
                <p><del><?php echo MONEDA . number_format($precio, 2, '.', ','); ?></p></del>
            ...
            <div class="d-grid gap-3 col-10 mx-auto">
                <a href="checkout_paquetes.php" class="btn btn-outline-secondary">Comprar ahora</a>
                <button class="btn btn-danger" type="button" onclick="addPaquete(<?php echo $id; ?>, '<?php echo $token_tmp; ?>')">Añadir a Combo Pedidos</button>
         </div>
        </div>
     </div>
    </div>
