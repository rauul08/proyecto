...
$db = new Database();
$con = $db->conectar();
$sql = $con->prepare("SELECT id, nombre, email, direccion_entrega, instrucciones_adicionales, metodo_pago, monto_total, proceso, fecha_realiza FROM pedidos WHERE proceso = 1");
$sql->execute();
            ...
            <table class="table table-bordered table-striped table-hover" style="width: 100%;" id="tblPedidos">
                ...
                if (count($resultado) == 0) {
                    ...
                    foreach ($resultado as $row) {
                        ...
                        //$proceso = $row['proceso'];
                        $fecha_realiza = $row['fecha_realiza'];
                ?>
                <tr data-id="<?php echo $id; ?>">
                    ...
                    <td><?php echo $fecha_realiza; ?></td>
                    <td><a href="#" class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#actualizarPedido" data-id="<?php echo $id; ?>"><i class="fa-solid fa-circle-up"></i></a></td>
...
<!-- Modal Update -->
<div class="modal fade" id="actualizarPedido" tabindex="-1" aria-labelledby="actualizarPedidoLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h1 class="modal-title fs-5" id="actualizarPedidoLabel">Tomar Pedido</h1>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        ¿Estás seguro de que deseas cambiar el estado de este pedido a "En Proceso"?
      </div>
...
document.addEventListener('DOMContentLoaded', function() {
  var actualizarPedido = document.getElementById('actualizarPedido');
  var confirmarCambio = document.getElementById('confirmarCambio');
  var pedidoId;

  actualizarPedido.addEventListener('show.bs.modal', function(event) {
